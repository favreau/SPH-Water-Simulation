...
SPHSolver::SPHSolver() {
  ...
  FloatRect particleRect =
      FloatRect((WIDTH - width) / 2, HEIGHT - height, width, height);

#if 1
  Vector2f aabb_min(1e10, 1e10);
  Vector2f aabb_max(-1e10, -1e10);

  float x, y, z;
  std::string line;
  std::ifstream myfile("/home/favreau/testdata.xyz");
  ...
  if (myfile.is_open()) {
    while (getline(myfile, line)) {
      ...
      aabb_min.x = std::min(aabb_min.x, x);
      aabb_min.y = std::min(aabb_min.y, y);
      aabb_max.x = std::max(aabb_max.x, x);
      aabb_max.y = std::max(aabb_max.y, y);
    }
    myfile.close();
  }

  Vector2f center((aabb_min.x + aabb_max.x) / 2.f,
                  (aabb_min.y + aabb_max.y) / 2.f);
  Vector2f size(aabb_max.y - aabb_min.x, aabb_max.y - aabb_min.y);
  //  std::cout << center.x << ", " << center.y << std::endl;
  //  std::cout << aabb_min.x << ", " << aabb_min.y << std::endl;
  //  std::cout << aabb_max.x << ", " << aabb_max.y << std::endl;
  //  std::cout << size.x << ", " << size.y << std::endl;

  for (const auto &position : positions) {
    Vector2f pos = Vector2f(1.5f + (position.x + center.x) / size.x,
  ...
  std::cout << numberParticles << std::endl;
  ...
  for (int i = 0; i < NUMBER_PARTICLES / 2.0f; i++) {
    for (int j = 0; j < NUMBER_PARTICLES; j++) {
      Vector2f pos = Vector2f(particleRect.left, particleRect.top) +
                     Vector2f(i * dx, j * dy);
...
void SPHSolver::render(RenderTarget &rt, Visualization vis) {
  ...
  if (vis != Visualization::Water) {
    ...
    sprite.setOrigin(metaballTexture.getSize().x / 2.0f,
                     metaballTexture.getSize().y / 2.0f);
  ...
  for (int i = 0; i < numberParticles; i++) {
    ...
    for (int n = 0; n < neighbors.size(); n++) {
      ...
      fPressure += particles[j].mass *
                   ...
                   (2.0f * particles[j].density) * gradKernel(x, KERNEL_RANGE);
      ...
      // Color field
      // particles[i].color += particles[j].mass / particles[j].density *
      // kernel(x, KERNEL_RANGE);
    }
